  ...
  describe("validateSchema", () => {
    ...
    describe("CLOSE", () => {
      it("returns same message if valid", () => {
        ...
        expect(result).not.to.have.property("error");
        expect(validateSchema(messageSchema)(message)).to.have.deep.property(
          "value",
          message,
        );
      });
      ...
      it("returns error if subscription ID is missing", () => {
        ...
        expect(result).to.have.nested.property(
          "error.message",
          '"CLOSE message" does not contain [subscriptionId]',
        );
      });
    });
    ...
    describe("REQ", () => {
      ...
      it("returns error if filter is missing", () => {
        ...
        expect(result).to.have.nested.property(
          "error.message",
          '"REQ message" does not contain [filter]',
        );
      });
      ...
      it("returns error if there are too many filters", () => {
        ...
        expect(result).to.have.nested.property(
          "error.message",
          '"REQ message" must contain less than or equal to 12 items',
        );
      });
    });
  });
