...
describe('SettingsStatic', () => {
    describe('.getSettingsFilePath', () => {
        ...
        it('returns string ending with .nostr/ by default', () => {
            expect(SettingsStatic.getSettingsFileBasePath()).to.be.a('string').and.to
                .match(/.nostr/)
        })

        it('returns path begins with user\'s home dir by default', () => {
            expect(SettingsStatic.getSettingsFileBasePath()).to.be.a('string').and
                .equal(`${join(Deno.cwd(), '.nostr')}`)
        })
    ...
    describe('.getDefaultSettingsFilePath', () => {
        ...
        it('returns string ending with settings.json by default', () => {
            expect(SettingsStatic.getDefaultSettingsFilePath()).to.be.a('string').and
                .to.match(/settings\.yaml$/)
        })

        it('returns path begins with user\'s home dir by default', () => {
            expect(SettingsStatic.getDefaultSettingsFilePath()).to.be.a('string').and
                .equal(`${join(Deno.cwd(), '/resources')}/default-settings.yaml`)
        })
    })
    ...
    describe('.loadAndParseYamlFile', () => {
        ...
        it('loads and parses yaml file from given path', () => {
            ...
            expect(SettingsStatic.loadAndParseYamlFile('/some/path/file.yaml')).to
                .equal('content')
    ...
    describe('.loadAndParseJsonFile', () => {
        ...
        it('loads and parses json file from given path', () => {
            ...
            expect(SettingsStatic.loadAndParseJsonFile('/some/path/file.json')).to
                .equal('content')
    ...
    describe('.loadSettings', () => {
        ...
        it('loads settings from given path', () => {
            ...
            expect(SettingsStatic.loadSettings('/some/path', SettingsFileTypes.yaml))
                .to.equal('content')
    ...
    describe('.createSettings', () => {
        ...
        beforeEach(() => {
            ...
            getSettingsFileBasePathStub = sandbox.stub(
                SettingsStatic,
                'getSettingsFileBasePath',
            )
            getDefaultSettingsFilePathStub = sandbox.stub(
                SettingsStatic,
        ...
        it('creates settings from defaults if settings file is missing', () => {
            ...
            expect(existsSyncStub).to.have.been.calledOnceWithExactly(
        ...
        it('loads settings from file if settings file exists', () => {
            ...
            expect(existsSyncStub).to.have.been.calledWithExactly(
                '/some/path/settings.yaml',
            )
            expect(getSettingsFileBasePathStub).to.have.been.calledOnce
            expect(saveSettingsStub).not.to.have.been.called
            expect(loadSettingsStub).to.have.been.calledWithExactly(
                '/some/path/settings.yaml',
                'yaml',
            )
        })
