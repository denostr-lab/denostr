...
describe("createNotice", () => {
  it("returns a notice message", () => {
    expect(createNoticeMessage("some notice")).to.deep.equal([
      MessageType.NOTICE,
      "some notice",
    ]);
  });
...
describe("createEndOfStoredEventsNoticeMessage", () => {
  it("returns a EOSE message", () => {
    expect(createEndOfStoredEventsNoticeMessage("subscriptionId")).to.deep
      .equal([MessageType.EOSE, "subscriptionId"]);
  });
});
