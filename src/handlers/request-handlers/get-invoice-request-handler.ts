...
export const getInvoiceRequestHandler = async (
  ctx: RouterContext,
  next: NextFunction,
) => {
  ...
  if (
    ...
    pathEq(
      ["payments", "feeSchedules", "admission", "0", "enabled"],
      true,
      settings,
    )
  ) {
    if (!pageCache) {
      ...
      const feeSchedule = path<FeeSchedule>([
        "payments",
        "feeSchedules",
        "admission",
        "0",
      ...
      pageCache = readFileSync("./resources/index.html", "utf8")
        ...
        .replaceAll(
          "{{amount}}",
          (BigInt(feeSchedule.amount) / 1000n).toString(),
        );
    }
