...
describe("EventRepository", () => {
  ...
  describe(".findByFilters", () => {
    ...
    describe("1 filter", () => {
      it("selects all events", () => {
        ...
        expect(query).to.equal(
          'select * from "events" order by "event_created_at" asc limit 500',
        );
      });
      ...
      describe("authors", () => {
        it("selects no events given empty list of authors", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where (1 = 0) order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by one author", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where ("event_pubkey" in (X\'22e804d26ed16b68db5259e78449e96dab5d464c8f470bda3eb1a70467f2c793\') or "event_delegator" in (X\'22e804d26ed16b68db5259e78449e96dab5d464c8f470bda3eb1a70467f2c793\')) order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by one author prefix (even length)", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where (substring("event_pubkey" from 1 for 3) = X\'22e804\' or substring("event_delegator" from 1 for 3) = X\'22e804\') order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by one author prefix (odd length)", () => {
          ...
          expect(query).to.equal(
            "select * from \"events\" where (substring(\"event_pubkey\" from 1 for 4) BETWEEN E'\\\\x22e804f0' AND E'\\\\x22e804ff' or substring(\"event_delegator\" from 1 for 4) BETWEEN E'\\\\x22e804f0' AND E'\\\\x22e804ff') order by \"event_created_at\" asc limit 500",
          );
        });
        ...
        it("selects events by two author prefix (first even, second odd)", () => {
          ...
          expect(query).to.equal(
            "select * from \"events\" where (substring(\"event_pubkey\" from 1 for 3) = X'22e804' or substring(\"event_delegator\" from 1 for 3) = X'22e804' or substring(\"event_pubkey\" from 1 for 4) BETWEEN E'\\\\x32e18270' AND E'\\\\x32e1827f' or substring(\"event_delegator\" from 1 for 4) BETWEEN E'\\\\x32e18270' AND E'\\\\x32e1827f') order by \"event_created_at\" asc limit 500",
          );
        });
      });
      ...
      describe("ids", () => {
        ...
        it("selects events by one id", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where ("event_id" in (X\'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\')) order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by two ids", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where ("event_id" in (X\'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\', X\'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\')) order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by one id prefix (even length)", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where (substring("event_id" from 1 for 2) = X\'abcd\') order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by one id prefix (odd length)", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where (substring("event_id" from 1 for 2) BETWEEN E\'\\\\xabc0\' AND E\'\\\\xabcf\') order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by two id prefix (first even, second odd)", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where (substring("event_id" from 1 for 3) = X\'abcdef\' or substring("event_id" from 1 for 2) BETWEEN E\'\\\\xabc0\' AND E\'\\\\xabcf\') order by "event_created_at" asc limit 500',
          );
        });
      });
      ...
      describe("kinds", () => {
        ...
        it("selects events by one kind", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where "event_kind" in (1) order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by two kinds", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where "event_kind" in (1, 2) order by "event_created_at" asc limit 500',
          );
        });
      });
      ...
      describe("since", () => {
        it("selects events since given timestamp", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where "event_created_at" >= 1000 order by "event_created_at" asc limit 500',
          );
        });
      });
      ...
      describe("limit", () => {
        it("selects 1000 events", () => {
          ...
          expect(query).to.equal(
            'select * from "events" order by "event_created_at" DESC limit 1000',
          );
        });
      });
      ...
      describe("#e", () => {
        ...
        it("selects events by one #e tag", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where ("event_tags" @> \'[["e","aaaaaa"]]\') order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by two #e tag", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where ("event_tags" @> \'[["e","aaaaaa"]]\' or "event_tags" @> \'[["e","bbbbbb"]]\') order by "event_created_at" asc limit 500',
          );
        });
      });
      ...
      describe("#p", () => {
        ...
        it("selects events by one #p tag", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where ("event_tags" @> \'[["p","aaaaaa"]]\') order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by two #p tag", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where ("event_tags" @> \'[["p","aaaaaa"]]\' or "event_tags" @> \'[["p","bbbbbb"]]\') order by "event_created_at" asc limit 500',
          );
        });
      });
      ...
      describe("#r", () => {
        ...
        it("selects events by one #r tag", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where ("event_tags" @> \'[["r","aaaaaa"]]\') order by "event_created_at" asc limit 500',
          );
        });
        ...
        it("selects events by two #r tag", () => {
          ...
          expect(query).to.equal(
            'select * from "events" where ("event_tags" @> \'[["r","aaaaaa"]]\' or "event_tags" @> \'[["r","bbbbbb"]]\') order by "event_created_at" asc limit 500',
          );
        });
      });
    });
    ...
    describe("2 filters", () => {
      it("selects union of both filters", () => {
        ...
        expect(query).to.equal(
          '(select * from "events") union (select * from "events" order by "event_created_at" asc limit 500) order by "event_created_at" asc limit 500',
        );
      });
    });
  ...
  describe("deleteByPubkeyAndIds", () => {
    it("marks event as deleted by pubkey & event_id if not deleted", () => {
      const query = repository.deleteByPubkeyAndIds("001122", [
        "aabbcc",
        "ddeeff",
      ]).toString();
  ...
  describe("upsert", () => {
    it("replaces event based on event_pubkey and event_kind", () => {
      const event: Event = {
        ...
        "sig":
          "d1de98733de2b412549aa64454722d9b66ab3c68e9e0d0f9c5d42e7bd54c30a06174364b683d2c8dbb386ff47f31e6cb7e2f3c3498d8819ee80421216c8309a9",
        [ContextMetadataKey]: { remoteAddress: { address: "::1" } as any },
      };
