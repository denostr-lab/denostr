...
export const isEventKindOrRangeMatch =
  ({ kind }: Event) => (item: EventKinds | EventKindsRange) =>
    typeof item === "number"
      ? item === kind
      : kind >= item[0] && kind <= item[1];

export const isEventMatchingFilter =
  (filter: SubscriptionFilter) => (event: Event): boolean => {
    ...
    if (Array.isArray(filter.authors)) {
      if (
        ...
        if (isDelegatedEvent(event)) {
          const delegation = event.tags.find((tag) =>
            tag[0] === EventTags.Delegation
          );
...
export const isDelegatedEventValid = async (event: Event): Promise<boolean> => {
  ...
  const serializedDelegationTag = `nostr:${delegation[0]}:${event.pubkey}:${
    delegation[2]
  }`;
...
export const getPublicKey = (privkey: string | Buffer) =>
  Buffer.from(secp256k1.getPublicKey(privkey, true)).subarray(1).toString(
    "hex",
  );
...
export const getEventExpiration = (event: Event): number | undefined => {
  const [, rawExpirationTime] =
    event.tags.find((tag) =>
      tag.length >= 2 && tag[0] === EventTags.Expiration
    ) ?? [];
