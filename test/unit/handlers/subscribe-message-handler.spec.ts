...
import {
  IAbortable,
  IMessageHandler,
} from "../../../src/@types/message-handlers.ts";
...
describe("SubscribeMessageHandler", () => {
  ...
  describe("#handleMessage", () => {
    ...
    beforeEach(() => {
      ...
      fetchAndSendStub = sandbox.stub(
        SubscribeMessageHandler.prototype,
        "fetchAndSend" as any,
      );
      canSubscribeStub = sandbox.stub(
        SubscribeMessageHandler.prototype,
        "canSubscribe" as any,
      );
    });
    ...
    it("emits subscribe event if subscription is accepted", async () => {
      ...
      expect(webSocketOnSubscribeStub).to.have.been.calledOnceWith(
        subscriptionId,
      );
      expect(fetchAndSendStub).to.have.been.calledOnceWithExactly(
        subscriptionId,
        filters,
      );
    });
  ...
  describe("#fetchAndSend", () => {
    ...
    beforeEach(() => {
      ...
      isClientSubscribedToEventStub = sandbox.stub(
        SubscribeMessageHandler,
        "isClientSubscribedToEvent" as any,
      );
    ...
    it("does not send event if client is not subscribed to it", async () => {
      ...
      expect(eventRepositoryFindByFiltersStub).to.have.been
        .calledOnceWithExactly(filters);
    });
    ...
    it("ends event stream if error occurs", async () => {
      ...
      const fetch = () =>
        (handler as any).fetchAndSend(subscriptionId, filters);
  ...
  describe(".isClientSubscribedToEvent", () => {
    it("returns false if event matches no filter", () => {
      ...
      expect(
        (SubscribeMessageHandler as any).isClientSubscribedToEvent(filters)(
          event,
        ),
      ).to.be.false;
    });
  ...
  describe("#canSubscribe", () => {
    it("returns undefined if subscription & filter count are allowed", () => {
      ...
      expect((handler as any).canSubscribe(subscriptionId, filters)).to.be
        .undefined;
    });
