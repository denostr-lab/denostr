...
export class Alternative implements IAlternative {
  ...
  public test(values: Record<string, any>): string | undefined {
    ...
    const why = (
      ...
      field: string,
      explanation: string,
    ): string | undefined => (cond) ? undefined : `${field}: ${explanation}`;
    ...
    switch (this.cond) {
      ...
      case "^":
        return why(
          val.startsWith(this.value),
          this.field,
          `does not start with ${this.value}`,
        );
      case "$":
        return why(
          val.endsWith(this.value),
          this.field,
          `does not end with ${this.value}`,
        );
      case "~":
        return why(
          values[this.field].includes(this.value),
          this.field,
      ...
      case "{":
        return why(
          val < this.value,
          this.field,
          `is the same or ordered after ${this.value}`,
        );
      case "}":
        return why(
          val > this.value,
          this.field,
          `is the same or ordered before ${this.value}`,
        );
    }
  }
  ...
  public static from(encodedStr: string): IAlternative {
    const [field, cond, value] = encodedStr.replace(/\s+/g, "").split(
      new RegExp(`(${punctuations.source})`, "g"),
    );
