...
describe("NIP-01", () => {
  ...
  describe("isEventMatchingFilter", () => {
    ...
    describe("kinds filter", () => {
      ...
      it("returns true if kinds filter contains event id", () => {
        expect(isEventMatchingFilter({ kinds: [event.kind] })(event)).to.be
          .true;
      });
  ...
  describe("isEventSignatureValid", () => {
    ...
    beforeEach(() => {
      event = {
        "id":
          "b1601d26958e6508b7b9df0af609c652346c09392b6534d93aead9819a51b4ef",
        "pubkey":
          "22e804d26ed16b68db5259e78449e96dab5d464c8f470bda3eb1a70467f2c793",
        ...
        "sig":
          "ec8b2bc640c8c7e92fbc0e0a6f539da2635068a99809186f15106174d727456132977c78f3371d0ab01c108173df75750f33d8e04c4d7980bbb3fb70ba1e3848",
      };
    });
  ...
  describe("#r filter", () => {
    beforeEach(() => {
      event = {
        ...
        content: "r",
        sig:
          "53d12018d036092794366283eca36df4e0cabd014b6e91bbf684c8bb9bbbe9dedafa77b6b928587e11e05e036227598dded8713e8da17d55076e12242b361542",
      };
    });
...
describe("NIP-16", () => {
  describe("isReplaceableEvent", () => {
    it("returns true if event is replaceable", () => {
      expect(isReplaceableEvent({ kind: EventKinds.REPLACEABLE_FIRST } as any))
        .to.be.true;
    });

    it("returns true if event is set_metadata", () => {
      expect(isReplaceableEvent({ kind: EventKinds.SET_METADATA } as any)).to.be
        .true;
    });

    it("returns true if event is contact_list", () => {
      expect(isReplaceableEvent({ kind: EventKinds.CONTACT_LIST } as any)).to.be
        .true;
    });

    it("returns false if event is not replaceable", () => {
      expect(
        isReplaceableEvent({ kind: EventKinds.REPLACEABLE_LAST + 1 } as any),
      ).to.be.false;
    });
  });
...
describe("NIP-26", () => {
  ...
  beforeEach(() => {
    event = {
      ...
      "content": "Hello world",
      "sig":
        "cd4a3cd20dc61dcbc98324de561a07fd23b3d9702115920c0814b5fb822cc5b7c5bcdaf3fa326d24ed50c5b9c8214d66c75bae34e3a84c25e4d122afccb66eb6",
    };
  });
...
describe("NIP-33", () => {
  describe("isParameterizedReplaceableEvent", () => {
    it("returns true if event is a parameterized replaceable event", () => {
      expect(isParameterizedReplaceableEvent({ kind: 30000 } as any)).to.be
        .true;
    });

    it("returns false if event is a parameterized replaceable event", () => {
      expect(isParameterizedReplaceableEvent({ kind: 40000 } as any)).to.be
        .false;
    });
  });
});
